...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entityToList(hub, hub_name, entities, initial_groups, hub.device_info,
    ...
    hub.rebuild_blocks(initial_groups) # first time call
    return True




def entityToList(hub, hub_name, entities, groups, device_info: DeviceInfo,
        ...
        if hub.plugin.matchInverterWithMask(hub._invertertype,sensor_description.allowedtypes, hub.seriesnumber, sensor_description.blacklist):
            ...
            if sensor_description.value_series is not None:
                for serie_value in range(sensor_description.value_series):
                    ...
                    entityToListSingle(hub, hub_name, entities, groups, device_info, newdescr, readPreparation, readFollowUp)
            else:
                newdescr = copy(sensor_description)
                try:
...
def entityToListSingle(hub, hub_name, entities, groups, device_info: DeviceInfo, newdescr, readPreparation, readFollowUp):  # noqa: D103
